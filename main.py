...
async def request(url: str):
    # Встановлюємо максимальний таймаут на виконання HTTP-запиту в 30 секунд.
    async with httpx.AsyncClient(timeout=30.0) as client:
        # Виконуємо GET-запит до вказаного URL
        r = await client.get(url)
        # Перевіряємо статус відповіді
        if r.status_code == 200:
            # Якщо статус 200 (OK), парсимо JSON-відповідь та повертаємо результат
            result = r.json()
            return result
        else:
            # Якщо отримано помилковий статус, піднімаємо власну помилку HttpError
